...
export default function PublicHome() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans flex flex-col">
      {/* Header */}
      <header className="absolute top-0 left-0 w-full z-50 border-b border-white/10 bg-[#0A192F]/80 backdrop-blur-md">
        <div className="container mx-auto px-4 lg:px-8 h-20 md:h-24 flex items-center justify-between">
          <div className="w-auto lg:w-1/4 flex justify-start">
            <Link to="/" className="flex items-center hover:opacity-80 transition-opacity">
              <img
                ...
                className="h-10 md:h-14 w-auto object-contain mix-blend-screen invert grayscale contrast-150"
              />
            </Link>
          </div>
            ...
            <nav className="flex gap-6 lg:gap-8 items-center">
              {navItems.map((item) => (
                <a
                  key={item.label}
                  href={item.href}
                  className="text-[10px] lg:text-xs font-medium tracking-[0.15em] text-white/80 hover:text-[#D4AF37] transition-colors uppercase"
                >
                  {item.label}
                </a>
              ))}
          ...
          <div className="w-auto xl:w-1/4 flex justify-end">
            ...
            <div className="hidden xl:flex items-center">
              <Link to="/login">
                <Button className="bg-transparent border border-white/20 text-white hover:bg-white hover:text-[#0A192F] tracking-widest uppercase text-[10px] font-bold h-10 px-6 rounded-none transition-all">
                  Acesso Restrito
                </Button>
              </Link>
            </div>
                  ...
                  <div className="flex flex-col h-full py-8">
                    <div className="px-8 mb-12 flex justify-center">
                      <img
                        src={logoImg}
                        alt="Nuvia Odontologia"
                        className="h-12 w-auto object-contain mix-blend-screen invert grayscale contrast-150"
                      />
                    </div>
                    <div className="flex-1 overflow-y-auto px-4 space-y-1">
                      {navItems.map((item) => (
                        <a
                          ...
                          className="flex items-center justify-between w-full p-4 text-xs font-semibold tracking-[0.2em] text-slate-300 hover:text-[#D4AF37] hover:bg-white/5 rounded-lg transition-colors uppercase"
                        >
                          {item.label}
                    ...
                    <div className="p-8 border-t border-slate-800 mt-auto">
                      <Link to="/login" onClick={() => setMobileMenuOpen(false)}>
                        <Button className="w-full bg-[#D4AF37] text-[#0A192F] hover:bg-[#C5A028] font-bold tracking-widest uppercase text-xs h-12 rounded-none">
                          Acesso Restrito
                        </Button>
                      </Link>
                    </div>
            ...
            <div className="col-span-1 md:col-span-2">
              <Link to="/" className="inline-block hover:opacity-80 transition-opacity">
                <img
                  src={logoImg}
                  alt="Nuvia Odontologia"
                  className="h-10 md:h-12 w-auto mb-6 object-contain mix-blend-screen invert grayscale contrast-150"
                />
              </Link>
