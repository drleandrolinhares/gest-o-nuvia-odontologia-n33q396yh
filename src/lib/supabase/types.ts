...
//     USING: ((usuario_id = auth.uid()) OR is_admin_user(auth.uid()) OR is_master_user(auth.uid()) OR (EXISTS ( SELECT 1    FROM (profiles p      JOIN cargos c ON ((p.cargo_id = c.id)))   WHERE ((p.id = auth.uid()) AND (c.nome = 'CEO'::text)))))
//   Policy "rotinas_execucao_insert" (INSERT, PERMISSIVE) roles={authenticated}
//     WITH CHECK: ((usuario_id = auth.uid()) OR is_admin_user(auth.uid()) OR is_master_user(auth.uid()) OR (EXISTS ( SELECT 1    FROM (profiles p      JOIN cargos c ON ((p.cargo_id = c.id)))   WHERE ((p.id = auth.uid()) AND (c.nome = 'CEO'::text)))))
