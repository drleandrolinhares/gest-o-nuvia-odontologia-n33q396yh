ALTER TABLE public.rotinas_execucao ADD COLUMN IF NOT EXISTS tipo_marcacao text DEFAULT 'colaborador';

DROP FUNCTION IF EXISTS public.registrar_execucao_rotina(uuid, uuid, date, numeric, integer);
DROP FUNCTION IF EXISTS public.registrar_execucao_rotina(uuid, uuid, date, numeric, integer, uuid);
DROP FUNCTION IF EXISTS public.registrar_execucao_rotina(uuid, uuid, date, numeric, integer, uuid, text);
...
CREATE OR REPLACE FUNCTION public.registrar_execucao_rotina(
    ...
    p_tipo_marcacao text DEFAULT 'colaborador'
)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
